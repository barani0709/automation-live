...
async function uploadToAzureBlobAndTable(directory, year, month) {
  ...
  for (const file of files) {
    const match = file.match(/^CRM_(.+?)_(\w+)-(\d{4})\.csv$/);
    if (!match) {
      ...
      try {
        ...
        for (const month of selectedMonths) {
          ...
          try {
            ...
            for (let i = 0; i <= 5; i++) {
              await page.locator(`#ctl00_CPH_chkDesignation_${i}`).check();//*[@id="ctl00_CPH_chkService"]//*[@id="ctl00_CPH_chkVisit"]//*[@id="ctl00_CPH_chkMonthWiseActivity"]
            }
            ...
            const filterCheckbox = page.locator('#ctl00_CPH_chkApplyFilter');
            await filterCheckbox.waitFor({ state: 'visible' });
            ...
            await page.locator('#ctl00_CPH_chkMonthWiseActivity').check();
            //*[@id="ctl00_CPH_ddeService_DDD_DDTC_lstService_0_D"]//*[@id="ctl00_CPH_ddeService_B-1Img"]

            const downloadPromise = page.waitForEvent('download', { timeout: 60000 });
            await page.locator('#ctl00_CPH_imgExcel').click();
            const download = await downloadPromise;

            const fileName = `CRM_${division}_${month}-${year}.csv`;
